...
internal sealed class DebugChlorophyteScan : ModSystem
{
    ...
    // ── Exposed for DebugStats ────────────────────────────────────────
    internal static int LastOre => _lastOre;
    internal static int LastBrick => _lastBrick;
    internal static int LastJungleGrass => _lastJungleGrass;
    internal static int LastDeepJungleGrass => _lastDeepJungleGrass;
    internal static int LastMud => _lastMud;
    internal static float ScanProgress
    {
        get
        {
            long total = (long)Main.maxTilesX * Main.maxTilesY;
            long idx = (long)_scanY * Main.maxTilesX + _scanX;
            return total <= 0 ? 0f : (float)idx / total;
        }
    }
