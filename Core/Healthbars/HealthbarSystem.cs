...
/// Inspiration:
/// https://github.com/WiseDragoon/TerrariaPlayersBars/blob/main/UI/HealthBar.cs
/// </summary>
[Autoload(Side = ModSide.Client)]
public class HealthbarSystem : ModSystem
{
    // Settings
    public bool IsActive = true; // if healthbars are shown
    public float Scale = 1.0f;
    public int Offset = 80; // y offset
    ...
    public override void OnWorldLoad()
    {
        ...
        Scale = cfg.HealthbarScale;
        Offset = cfg.HealthbarYOffset;
    }
    ...
    private sealed class HealthBarLayer : GameInterfaceLayer
    {
        ...
        protected override bool DrawSelf()
        {
            ...
            for (int i = 0; i < Main.maxPlayers; i++)
            {
                ...
                if (cfg.ShowManaBars)
                    DrawManabar(player, scale, offset, sb);
        ...
        private void DrawHealthbar(Player player, float scale, float offset, SpriteBatch sb)
        {
            // Health ratio
            ...
            {
                if (barPixels < 36)
                    sb.Draw(back, pos + new Vector2(barPixels * scale, 0f), new Rectangle(2, 0, 2, back.Height), color, 0f, Vector2.Zero, scale, SpriteEffects.None, 0f);

                if (barPixels < 34)
                    sb.Draw(back, pos + new Vector2((barPixels + 2) * scale, 0f), new Rectangle(barPixels + 2, 0, 36 - barPixels - 2, back.Height), color, 0f, Vector2.Zero, scale, SpriteEffects.None, 0f);

                if (barPixels > 2)
                    sb.Draw(fill, pos, new Rectangle(0, 0, barPixels - 2, fill.Height), color, 0f, Vector2.Zero, scale, SpriteEffects.None, 0f);

                sb.Draw(fill, pos + new Vector2((barPixels - 2) * scale, 0f), new Rectangle(32, 0, 2, fill.Height), color, 0f, Vector2.Zero, scale, SpriteEffects.None, 0f);
            }
            else
            {
                if (barPixels < 36)
                    sb.Draw(back, pos + new Vector2(barPixels * scale, 0f), new Rectangle(barPixels, 0, 36 - barPixels, back.Height), color, 0f, Vector2.Zero, scale, SpriteEffects.None, 0f);

                sb.Draw(fill, pos, new Rectangle(0, 0, barPixels, fill.Height), color, 0f, Vector2.Zero, scale, SpriteEffects.None, 0f);
            }
        }
        private void DrawManabar(Player player, float scale, float offset, SpriteBatch sb)
        {
            if (player.statManaMax2 <= 0)
                return;

            float ratio = Utils.Clamp((float)player.statMana / player.statManaMax2, 0f, 1f);
            if (ratio <= 0f)
                return;
            ...
            Vector2 pos = player.Top + new Vector2(-w * scale / 2f, offset - h * scale) - Main.screenPosition;
            pos.Y -= 10f * scale;
            pos.Y += (h + 2f) * scale; // stack below health bar
    ...
    //[Obsolete("Used for drawing custom health bars. Vanilla is used temporarily for now.")]
