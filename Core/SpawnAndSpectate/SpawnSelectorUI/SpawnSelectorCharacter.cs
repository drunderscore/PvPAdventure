...
internal class SpawnSelectorCharacter : UIPanel
{
    ...
    public SpawnSelectorCharacter(int _playerIndex)
    {
        // Set player index
    ...
    public override void MouseOut(UIMouseEvent evt)
    {
        ...
        if (SpawnAndSpectateSystem.HoveredPlayerIndex == playerIndex)
            SpawnAndSpectateSystem.HoveredPlayerIndex = -1;
    }
    ...
    public override void LeftClick(UIMouseEvent evt)
    {
        ...
        if (!SpawnAndSpectateSystem.IsValidTeammateTarget(playerIndex, requireAlive: true))
            return;

        if (SpawnAndSpectateSystem.IsRespawnFrozen(local))
        {
            local.GetModPlayer<SpawnAndSpectatePlayer>().CommitTeammateRespawn(playerIndex);
            return;
        }

        SpawnAndSpectateSystem.ToggleSpawnSelection(playerIndex);
    }
    ...
    protected override void DrawSelf(SpriteBatch sb)
    {
        ...
        if (IsMouseHovering)
        {
            ...
            if (!player.dead)
            {
                bool frozen = Main.LocalPlayer.dead && Main.LocalPlayer.respawnTimer == 2;
                bool selected = SpawnAndSpectateSystem.SelectedSpawnPlayerIndex == player.whoAmI;

                string text = frozen
                    ? $"Left click to respawn on {player.name}"
                    : selected
                        ? Language.GetTextValue("Mods.PvPAdventure.SpawnAndSpectate.SelectedSpawn", player.name)
                        : Language.GetTextValue("Mods.PvPAdventure.SpawnAndSpectate.SelectSpawn", player.name);
