...
/// Includes, for example <see cref="SpectateCharacter"/>, and <see cref="SpectateExitPanel"/>.
/// </summary>
public class SpectateBasePanel : UIPanel
{
    // UI components
    private SpectateExitPanel exitPanel;

    // Collections
    private readonly List<SpectateCharacter> playerItems = []; // list of teammate player UI items
    ...
    private void Rebuild()
    {
        ...
        float itemWidth = SpectateCharacter.ItemWidth;
        float itemHeight = SpectateCharacter.ItemHeight;
        ...
        for (int i = 0; i < playerCount; i++)
        {
            ...
            var row = new SpectateCharacter(players[i]);
            row.Left.Set(x, 0f);
            row.Top.Set(y, 0f);
    ...
    private bool NeedsRebuild()
    {
        ...
        var randomDims = exitPanel.GetDimensions();
