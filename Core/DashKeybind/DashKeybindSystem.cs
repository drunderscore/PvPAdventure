...
using static PvPAdventure.Core.DashKeybind.DashKeybindSystem;

namespace PvPAdventure.Core.DashKeybind;

/// <summary>
/// Provides a mod system that enables custom dash keybind handling for players, allowing for configurable dash behavior
/// and network synchronization in multiplayer environments.
/// </summary>
/// <remarks>This system intercepts and manages dash actions based on user configuration, supporting both vanilla
/// and custom dash logic. It ensures that dash actions are correctly handled and synchronized between clients and
/// servers in multiplayer games.
/// Integration of ModKeybind implementation in <see cref="Keybinds"/>
/// </remarks>
...
public class DashKeybindPlayer : ModPlayer
{
    ...
    public override void ProcessTriggers(TriggersSet triggers)
    {
        ...
        Log.Debug($"ProcessTriggers: player {Player.whoAmI} requesting dash dir={inputDirection} dashType={Player.dashType} delay={Player.dashDelay}");

        if (PerformDash(inputDirection, force: false, out byte dashTypeUsed))
        {
            Log.Debug($"Dash executed locally on {Player.whoAmI} using type {dashTypeUsed} (netMode={Main.netMode}).");
            if (Main.netMode == NetmodeID.MultiplayerClient)
            {
    ...
    internal bool PerformDash(int direction, bool force, out byte dashTypeUsed, int? overrideDashType = null)
    {
        ...
        {
            Log.Debug($"PerformDash aborted: player {Player.whoAmI} has no dash-granting item (override={overrideDashType}).");
    ...
    {
        Log.Debug($"SendDashRequest: player {Player.whoAmI} dir={direction} type={dashTypeUsed}");
    ...
    {
        Log.Debug($"BroadcastDash: player {Player.whoAmI} dir={direction} type={dashTypeUsed}");
        if (Main.netMode != NetmodeID.Server)
            return;
