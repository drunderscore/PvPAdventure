...
{
    public static void Send(int playerId, bool hasPortal, Vector2 worldPos, int toWho = -1, int ignoreClient = -1)
    {
    ...
    public static void HandlePacket(BinaryReader reader, int whoAmI)
    {
        ...
        player.GetModPlayer<SpawnPlayer>().ApplyPortalFromNet(hasPortal, worldPos);

        if (Main.netMode == NetmodeID.Server)
            Send(playerId, hasPortal, worldPos, toWho: -1, ignoreClient: whoAmI);
    }
}
